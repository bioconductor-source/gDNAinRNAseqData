.gDNAinRNAseqDataFromUrl <- function(baseUrl) {
...
makeMetadata_gDNAinRNAseqData <- function()
{
  biocver <- as.character(BiocManager::version())
  baseUrl <- "https://functionalgenomics.upf.edu/experimenthub/gdnainrnaseqdata/"
  meta <- .gDNAinRNAseqDataFromUrl(baseUrl)
  n <- nrow(meta)
  data.frame(
    Title=meta$title,
...
metadata <- makeMetadata_gDNAinRNAseqData()
write.csv(metadata, file="../extdata/metadata_gDNAinRNAseqData.csv", row.names=FALSE)
